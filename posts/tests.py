...
@pytest.fixture
def other_user(db):
    user = User.objects.create_user(username='otheruser', email='otheruser@example.com', password='TestPassword123!')
    return user

@pytest.fixture
def authenticated_client(api_client, create_user):
    response = api_client.post('/api/auth/login/', {'username': 'testuser', 'password': 'TestPassword123!'})
    api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {response.data["access"]}')
    return api_client

@pytest.fixture
def authenticated_client_other(api_client, other_user):
    response = api_client.post('/api/auth/login/', {'username': 'otheruser', 'password': 'TestPassword123!'})
    api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {response.data["access"]}')
    return api_client
...
def test_only_author_can_update_post(authenticated_client_other, create_post):
    response = authenticated_client_other.put(f'/api/posts/{create_post.id}/', {
        'title': 'Hacked Post',
    }, format='json')
