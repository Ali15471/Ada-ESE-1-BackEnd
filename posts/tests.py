...
@pytest.fixture
def other_user(db):
    user = User.objects.create_user(username='otheruser', email='otheruser@example.com', password='otherpassword')
    return user
...
def authenticated_client_other(api_client, other_user):
    response = api_client.post('/api/auth/login/', {'username': 'otheruser', 'password': 'otherpassword'})
    api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {response.data["access"]}')
    return api_client

@pytest.fixture
def create_post(db, create_user):
    post = Post.objects.create(title='Test Post', content='This is a test post.', author=create_user, status='published')
    return post

@pytest.fixture
def create_draft_post(db, create_user):
    post = Post.objects.create(title='Draft Post', content='This is a draft post.', author=create_user, status='draft')
    return post
...
@pytest.mark.django_db
def test_only_author_can_update_post(other_user, create_post):
    response = other_user.put(f'/api/posts/{create_post.id}/', {
        'title': 'Hacked Post',
    }, format='json')
...
@pytest.mark.django_db
def test_only_author_can_delete_post(other_user, create_post):
    response = other_user.delete(f'/api/posts/{create_post.id}/')
    assert response.status_code == 403
